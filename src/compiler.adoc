...
Then, write the following content into `index.html` file:
...
<html>
  ...
  <body>
    ...
    <script src="out/leapyears.js" type="text/javascript"></script>
  </body>
</html>
----
...
[source, bash]
----
mkdir -p vendor/leapyears/vendor/
touch vendor/leapyears/vendor/util_closure.js
----

And add the implementation using closure module format:

.vendor/leapyears/vendor/util_closure.js
[source, js]
----
...
[source, clojure]
----
(require '[leapyears.vendor.util-closure :as util])
...
So, if don't find our library in cljsjs, we can include it using the same
